...
namespace SVGImporter.Utils
{
	...
	public struct SVGColorExtractor
	{
		...
		{
			if (colorStr.Length > 0 && colorStr[0] == '#' && (colorStr.Length == 4 || colorStr.Length == 7))
			{
				return true;
			}
			return false;
		...
		{
			if (colorStr.Length > 0 && colorStr.ToLower().Contains("rgb"))
			{
				return true;
			}
			return false;
		...
		public static Color HexColor(string colorStr)
		{
			...
			if (colorStr.Length > 0 && colorStr[0] == '#')
			{
				...
				{
					int num = ParseHexDigit(colorStr[1]);
					r = num * 16 + num;
					num = ParseHexDigit(colorStr[2]);
					g = num * 16 + num;
					num = ParseHexDigit(colorStr[3]);
					b = num * 16 + num;
				}
			...
			if (colorStr.Length > 0)
			{
				...
				if (num != -1)
				{
					...
					if (!string.IsNullOrEmpty(colorStr))
					{
						...
						{
							r = GetColorValue(array[0]);
							if (array.Length > 1)
							{
								g = GetColorValue(array[1]);
							}
							if (array.Length > 2)
							{
								b = GetColorValue(array[2]);
							}
						}
					}
				}
